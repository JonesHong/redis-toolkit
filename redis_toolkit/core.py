...
class RedisToolkit:
    ...
    def _init_redis_client(
        ...
        config: Optional[RedisConnectionConfig]
    ):
        """初始化 Redis 客戶端"""
    ...
    def setter(self, name: str, value: Any, options: Optional[RedisOptions] = None) -> None:
        """
    ...
    def batch_set(self, mapping: Dict[str, Any], options: Optional[RedisOptions] = None) -> None:
        """
        批次設定鍵值對
        ...
        try:
            ...
            raise SerializationError(
                f"批次設定 {len(mapping)} 個鍵失敗", 
                original_exception=e
            ) from e
    
    def batch_get(self, names: List[str], options: Optional[RedisOptions] = None) -> Dict[str, Any]:
        """
        批次取得鍵值
        ...
        try:
            ...
            raise SerializationError(
                f"批次取得 {len(names)} 個鍵失敗", 
                original_exception=e
            ) from e
    
    def publisher(self, channel: str, data: Any, options: Optional[RedisOptions] = None) -> None:
        """
        發布訊息到指定頻道
